...
export const ContactForm = () => {
	...
	function handleSubmit(ev: React.FormEvent) {
		...
		const q = vulnerable ? "?vulnerable=1" : ""
		window.location.href = `/show-message${q}`
	}

	const inputStyles = "mt-1 block w-full rounded border p-2 border-gray-400/40 rounded-md"
	...
	return (
		<form onSubmit={handleSubmit} className="flex flex-col items-center gap-4">
			...
			<label className="block w-full">
				<span className="block text-sm">Mensaje</span>
				<textarea
					required
					value={message}
			...
			<label className="flex items-center gap-2 self-start text-sm">
				...
				<span>Simule vulnerable page</span>
			</label>
